...
namespace PSFilterLoad.PSApi
{
    ...
    internal sealed class LoadPsFilter : IDisposable
    {
        ...
        static bool isFM_FF;
        /// <summary>
        /// Determines whether the filter is a Filter Meister or Filter Factory plugin.
        /// </summary>
        /// <param name="data">The <c>PluginData</c> to check.</param>
        /// <returns>
        ///   <c>true</c> if the filter is a Filter Meister or Filter Factory plugin; otherwise, <c>false</c>.
        /// </returns>
        static bool IsFMOrFFPlugin(PluginData data)
        {
            bool isFF = !NativeMethods.EnumResourceNames(data.entry.dll, "PARM", new EnumResNameDelegate(delegate(IntPtr hModule, IntPtr lpszType, IntPtr lpszName, IntPtr lParam)
            {

                IntPtr find = NativeMethods.FindResource(hModule, lpszName, lpszType);
                if (find == IntPtr.Zero)
                {
                    return true;
                }
            ...
            bool isFMCode = !NativeMethods.EnumResourceNames(data.entry.dll, "FMCODE", new EnumResNameDelegate(delegate(IntPtr hModule, IntPtr lpszType, IntPtr lpszName, IntPtr lParam)
            {
            ...
            bool isFMDlg = !NativeMethods.EnumResourceNamesWithID(data.entry.dll, NativeConstants.RT_DIALOG, new EnumResNameDelegate(delegate(IntPtr hModule, IntPtr lpszType, IntPtr lpszName, IntPtr lParam)
            {


                if (Marshal.PtrToStringAnsi(lpszName) == "FILTERPARAM")
                {
                    return true;
                }
                return false;
        ...
        static void save_parm()
        {
            if (filterRecord.parameters != IntPtr.Zero)
            {
                ...
                {
                    parmData.ParmHandle = filterRecord.parameters;
                    parmData.HandleSize = handle_get_size_proc(filterRecord.parameters);

                    if (size == 8 && Marshal.ReadInt32(filterRecord.parameters, 4) == 0x464f544f)
                    {
                        int paddr = Marshal.ReadInt32(parmData.ParmHandle);
                ...
                {
                    parmData.ParmHandle = NativeMethods.GlobalLock(filterRecord.parameters);
        ...
        static void restore_parm()
        {
            ...
            if (parmData.ParmHandle != IntPtr.Zero && parmData.ParmDataBytes != null)
            {
                ...
                switch (parmData.StoreMethod)
                {
                    case 0:
                        ...
                        if (parmData.HandleSize == 8 && parmData.ParmDataIsPSHandle)
                        {
                            filterRecord.parameters = handle_new_proc((int)parmData.HandleSize);
                    ...
                    case 2:
                        ...
                        if (parmData.HandleSize == 8 && parmData.ParmDataIsPSHandle)
                        {
                            filterRecord.parameters = NativeMethods.GlobalAlloc(NativeConstants.GMEM_MOVEABLE, new UIntPtr((uint)parmData.HandleSize));
            ...
            if (parmData.PluginData != IntPtr.Zero && parmData.PluginDataBytes != null)
            {
