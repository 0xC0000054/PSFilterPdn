    ...
    internal sealed class EnableInfoInterpreter : IExpressionVisitor
    {
            ...
            if (Visit(node.Left) is ConstantExpression left && Visit(node.Right) is ConstantExpression right)
            {
                ...
                else
                {
                    ...
                    switch (node.NodeType)
                    {
                        ...
                        default:
                            throw new EnableInfoException(string.Format(CultureInfo.InvariantCulture,
                                                                        "{0} is not a supported binary operator.",
                                                                        node.NodeType));
                    }
                }
            }
        ...
        private BooleanConstantExpression VisitInFunction(ReadOnlyCollection<Expression> args)
        {
            if (args != null && args.Count >= 2)
            {
                ...
                if (Visit(args[0]) is ConstantExpression first)
                {
                    if (first.ConstantType == ConstantType.String)
                    {
                        ...
                        {
                            if (Visit(args[i]) is StringConstantExpression parameter
                                && string.Equals(firstParameter, parameter.Value, StringComparison.OrdinalIgnoreCase))
                            {
                    ...
                    else if (first.ConstantType == ConstantType.Integer)
                    {
                        ...
                        for (int i = 1; i < args.Count; i++)
                        {
                            if (Visit(args[i]) is IntegerConstantExpression parameter && firstParameter == parameter.Value)
                            {
                                return BooleanConstantExpression.True;
                            }
                        }
                    ...
                    else if (first.ConstantType == ConstantType.Boolean)
                    {
                        ...
                        for (int i = 1; i < args.Count; i++)
                        {
                            if (Visit(args[i]) is BooleanConstantExpression parameter && firstParameter == parameter.Value)
                            {
                                return BooleanConstantExpression.True;
                            }
                        }
        ...
        private IntegerConstantExpression VisitMinFunction(ReadOnlyCollection<Expression> args)
        {
            ...
            if (args != null && args.Count >= 2)
            {
                if (Visit(args[0]) is IntegerConstantExpression first && Visit(args[1]) is IntegerConstantExpression second)
                {
                    min = Math.Min(first.Value, second.Value);

                    for (int i = 2; i < args.Count; i++)
                    {
                        if (Visit(args[i]) is IntegerConstantExpression parameter)
                        {
        ...
        private BooleanConstantExpression VisitDimFunction(ReadOnlyCollection<Expression> args)
        {
            ...
            if (args != null && args.Count == 2)
            {
                ...
                if (Visit(args[0]) is BooleanConstantExpression first && Visit(args[1]) is BooleanConstantExpression second)
                {
        ...
        {
            if (Visit(node.Left) is BooleanConstantExpression left)
            {
                switch (node.NodeType)
                {
                    ...
                    default:
                        throw new EnableInfoException(string.Format(CultureInfo.InvariantCulture,
                                                                    "{0} is not a supported logical expression.",
                                                                    node.NodeType));
                }

                if (Visit(node.Right) is BooleanConstantExpression right)
                {
                    return right;
                }
            }
            ...
            if (Visit(node.Expression) is ConstantExpression expression)
            {
                ...
                else if (node.NodeType == ExpressionType.Negate || node.NodeType == ExpressionType.UnaryPlus)
                {
                    ...
                    switch (node.NodeType)
                    {
                        ...
                        default:
                            throw new EnableInfoException(string.Format(CultureInfo.InvariantCulture,
                                                                        "{0} is not a supported unary operator.",
                                                                        expression.NodeType));
                    }
                }
                else
