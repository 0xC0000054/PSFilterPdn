...
namespace PSFilterLoad.PSApi
{
    internal sealed unsafe class LoadPsFilter : IDisposable, IFilterImageProvider, IPICASuiteDataProvider
    {
        ...
        private unsafe short FillOutputBuffer(FilterRecord* filterRecord)
        {
            ...
            FilterPadding padding = new(rect, width, height, source.Size, null);
        ...
        private unsafe short FillMaskBuffer(FilterRecord* filterRecord)
        {
            ...
            FilterPadding padding = new(rect, width, height, source.Size, filterRecord->maskRate);
        ...
        private unsafe void SetPostProcessingOptions()
        {
        ...
        private static unsafe void SetFilterEdgePadding(IntPtr inData, int inRowBytes, Rect16 rect, int nplanes, short ofs, FilterPadding padding, Surface surface)
        {
            ...
            if (top > 0)
            {
                for (int y = 0; y < top; y++)
                {
                    ColorBgra* src = surface.GetRowAddressUnchecked(0);
            ...
            if (bottom > 0)
            {
                ...
                for (int y = 0; y < bottom; y++)
                {
                    ColorBgra* src = surface.GetRowAddressUnchecked(lastSurfaceColumn);
