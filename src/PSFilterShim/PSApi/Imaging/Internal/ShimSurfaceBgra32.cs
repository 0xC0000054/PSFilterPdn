...
namespace PSFilterLoad.PSApi.Imaging.Internal
{
    internal sealed unsafe class ShimSurfaceBgra32 : ImageSurface
    {
        ...
        /// </remarks>
        private unsafe void CopySurface(ShimSurfaceBgra32 source)
        {
                ...
                height == source.height)
            {
                NativeMemory.Copy(source.scan0.VoidStar,
                            scan0.VoidStar,
                            checked(((nuint)(height - 1) * (nuint)stride) + ((nuint)width * (nuint)sizeof(ColorBgra32))));
            }
            else
            {
                ...
                for (int y = 0; y < copyHeight; ++y)
                {
                    NativeMemory.Copy(source.GetRowAddressUnchecked(y),
                                      ...
                                      (nuint)copyWidth * (nuint)sizeof(ColorBgra32));
                }
            }
        }
